"""
mainly from https://github.com/shamangary/Pytorch-Stochastic-Depth-Resnet/blob/master/TYY_stodepth_lineardecay.py
"""
...
def buildModel(numClasses=10, isFlex=False):
    # net = resnet50_StoDepth_lineardecay(pretrained=True, prob_0_L=[1, 0.5], multFlag=True)
    net = torchvision.models.resnet50(pretrained=True)

    # modify the final fc layer to fit the numClasses, and add dropout layers
    fc_inputs = net.fc.in_features
    hidden_layers = (fc_inputs+numClasses) // 2
    net.fc = nn.Sequential(nn.Linear(fc_inputs, hidden_layers), nn.Dropout(0.5),
                nn.Linear(hidden_layers, hidden_layers), nn.Dropout(0.5),
                nn.Linear(hidden_layers, numClasses)
    )
    if isFlex:
        net.fc = nn.Sequential(nn.Linear(fc_inputs, hidden_layers),
